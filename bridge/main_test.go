...
func newTestBridge(t *testing.T, baseURI, eventsURI string) *Bridge {
	...
	bridge := &Bridge{
		...
		// The poll URI carries this, and the org refuses a non-positive value. Left at
		// zero it would make every drain in these tests a bad request.
		maxEventsPerDrain: MAX_EVENTS_PER_DRAIN,
